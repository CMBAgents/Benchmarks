...
def make_prompts():
    ...
    def delensing_efficiency():
        lmax = 2000
        l, n0 = np.loadtxt('./data/N0.csv',delimiter=',', skiprows=1).T
        l = l[:lmax+1]
        n0 = n0[:lmax+1]
...
def get_prompts(verbose=False):
    if os.path.isfile(filename):
        if verbose:
            print(f"Loading prompts from file({__version__}):", filename)
        df = pd.read_csv(filename)
    else:
        if verbose:
            print("Prompts file not found, creating the dataframe.")
