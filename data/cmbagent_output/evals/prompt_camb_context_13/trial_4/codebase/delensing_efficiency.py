...
def main():
    ...
    3. Load the lensing noise power spectrum N0 from CSV and align it with CAMB output.
    ...
    7. Calculate the delensing efficiency for multipoles l=2 to 100.
    8. Save the results in a CSV file under the 'data/' directory and print a summary.
    """
    ...
    lmax_calc = 3000  # For internal accuracy in CAMB
    lens_potential_accuracy_val = 2  # Lensing accuracy (higher is more accurate)
    lmax_out = 2000  # Output up to this multipole
    ...
    # --- 6. Delensed B-mode power spectrum ---
    cls_delensed = results.get_lensed_cls_with_spectrum(clpp=cl_pp_res_padded, CMB_unit='muK', lmax=lmax_out)
    dl_bb_delensed = cls_delensed[:, 2]
