...
Save the result in data/result.csv with columns: l, TT (μK^2).
"""
...
def compute_delensed_tt_cls(
    ...
    delens_efficiency=0.8,  # Delensing efficiency (fraction of lensing removed)
    output_csv="data/result.csv"
):
    r"""
    Compute the delensed CMB temperature power spectrum (l(l+1)C_l^{TT}/(2π)) in μK^2
    for multipoles l=lmin to lmax, using CAMB, and save to CSV.
    ...
    lensed_cl = powers['total'][:, 0]           # TT lensed, units: μK^2
    unlensed_cl = powers['unlensed_scalar'][:, 0]  # TT unlensed, units: μK^2

    # l values corresponding to the spectra
    ...
    delensed_cl = unlensed_cl + (lensed_cl - unlensed_cl) * (1.0 - delens_efficiency)
    ...
    print("Columns: l (multipole), TT (delensed l(l+1)C_l^{TT}/(2π) in μK^2)")
    print("Multipole range: " + str(lmin) + " to " + str(lmax))
    print("First 5 rows:")
    print(df.head())
    print("Last 5 rows:")
    print(df.tail())
