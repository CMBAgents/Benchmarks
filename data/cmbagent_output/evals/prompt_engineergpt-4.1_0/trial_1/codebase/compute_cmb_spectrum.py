...
def compute_cmb_tt_spectrum():
    ...
    Returns:
        ...
        tt_spectrum (np.ndarray): Temperature power spectrum l(l+1)C_l^{TT}/(2pi) in microkelvin^2, shape (N,).
    """
    ...
    # l values: cl[ell] corresponds to ell, for ell in 0..lmax
    l_vals = np.arange(lmin, lmax+1)
    # TT spectrum: cl[:,0] is C_l^{TT} in (muK)^2
    cl_tt = cl[lmin:lmax+1, 0]  # (lmax-lmin+1,)

    # Compute l(l+1)C_l^{TT}/(2pi) in (muK)^2
...
def save_spectrum_to_csv(l_vals, tt_spectrum, filename):
    ...
    Args:
        ...
        tt_spectrum (np.ndarray): TT spectrum, shape (N,).
...
if __name__ == "__main__":
    ...
    save_spectrum_to_csv(l_vals, tt_spectrum, output_file)