# filename: codebase/camb_matter_power.py
import numpy as np
import pandas as pd
import camb
...
def compute_pk(cosmo_params, k_h, hierarchy):
    ...
    -------
    pk : ndarray
        Linear matter power spectrum at z=0 in units of (Mpc/h)^3, evaluated at k_h.
    """
...
def main():
    ...
    between inverted and normal neutrino hierarchies for given cosmological parameters.
    """
    ...
    # k grid: 200 points, 1e-4 < k h < 2 [h/Mpc]
    ...
    print("Relative difference in linear matter power spectrum (P_inverted / P_normal - 1) saved to data/result.csv")
    print("First 10 rows:")
    print(df.head(10).to_string(index=False))
    print("\nColumns:")
    print(list(df.columns))
    print("k: Wavenumber in h/Mpc (200 values from 1e-4 to 2)")
    print("rel_diff: Relative difference in P(k) between inverted and normal hierarchy")
