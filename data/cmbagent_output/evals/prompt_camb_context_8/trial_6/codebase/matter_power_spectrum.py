...
def compute_linear_matter_power_spectrum():
    ...
    print("Linear matter power spectrum at z=0 calculated and saved to data/result.csv")
    print("kh (h/Mpc) range: " + str("{:.5e}" % kh.min()) + " to " + str("{:.5e}" % kh.max()))
    print("P(k) ((Mpc/h)^3) range: " + str("{:.5e}" % P_k_values.min()) + " to " + str("{:.5e}" % P_k_values.max()))
    print("First 5 rows (kh [h/Mpc], P_k [(Mpc/h)^3]):")
    for i in range(5):
        print(str("{:.5e}" % kh[i]) + ", " + str("{:.5e}" % P_k_values[i]))
