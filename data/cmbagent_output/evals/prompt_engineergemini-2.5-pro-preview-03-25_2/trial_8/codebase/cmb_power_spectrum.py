...
def calculate_cmb_power_spectrum():
    ...
    # With raw_cl=True, this returns C_l in muK^2 when CMB_unit='muK'.
...
def main():
    ...
    if valid_selected_ls:
        # Create a new DataFrame for selected values to avoid SettingWithCopyWarning
        selected_values_df = cmb_df[cmb_df['l'].isin(valid_selected_ls)].copy()
        
        # Set pandas options for printing full content
        pd.set_option('display.max_rows', None) # Show all rows for selected values
