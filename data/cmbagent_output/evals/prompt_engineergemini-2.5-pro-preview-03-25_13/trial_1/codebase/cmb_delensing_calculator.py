# filename: codebase/cmb_delensing_calculator.py
import os
import numpy as np
import pandas as pd
try:
...
class CMBDelensingCalculator:
    ...
    def calculate_residual_cl_phiphi(self, cl_phiphi_scaled, nl_phiphi_scaled):
        ...
        # valid_calc_mask ensures we only operate where ells >= 2 and denominator != 0
        ...
        for l_val in [2, 10, 20, 50, 100]:
            if l_val <= output_lmax:
                ...
                if len(idx) > 0:
                    ...
                    print(str(l_val).ljust(11) + " | " + "{:.4f}".format(eff))
