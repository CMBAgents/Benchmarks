...
def calculate_delensing_efficiency():
    ...
    # powers are D_ell = ell*(ell+1)*C_ell/(2*pi) for TT,EE,BB,TE
    ...
    # 2. CMB lensing potential power spectrum (C_ell^phiphi * (ell*(ell+1))^2 / (2*pi))
    ...
    # Requested quantity: C_ell^phiphi * (ell*(ell+1))^2 / (2*pi)
    cl_kappa_like = camb_cl_phi_phi * (ells_for_kappa_like * (ells_for_kappa_like + 1))**2 / (2 * np.pi)
    cl_kappa_like[0:2] = 0.0 

    print("\nCMB lensing potential related spectrum C_ell^phiphi * (ell*(ell+1))^2 / (2*pi):")
    ...
    # get_delensed_scalar_cls returns D_ell = ell*(ell+1)*C_ell/(2*pi)
