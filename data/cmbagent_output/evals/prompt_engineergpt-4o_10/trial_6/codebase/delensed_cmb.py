...
delensed_cl[:, 0] *= delensing_efficiency

# Calculate the delensed temperature power spectrum
ell = np.arange(2, lensed_cl.shape[0])
delensed_tt = ell * (ell + 1) * delensed_cl[2:, 0] / (2 * np.pi)

# Save results to CSV
data = pd.DataFrame({'l': ell, 'TT': delensed_tt})
