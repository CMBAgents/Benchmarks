...
delensed_cl = lensed_cl.copy()
delensed_cl[:, 0] *= (1 - delensing_efficiency)

# Prepare data for saving
l_values = np.arange(2, 3001)
tt_values = delensed_cl[2:3001, 0] * l_values * (l_values + 1) / (2 * np.pi)

# Save to CSV
data = pd.DataFrame({'l': l_values, 'TT': tt_values})
...
print("The file contains two columns: 'l' (multipole moment) and 'TT' (delensed temperature power spectrum in \u03bcmK\u00b2).")