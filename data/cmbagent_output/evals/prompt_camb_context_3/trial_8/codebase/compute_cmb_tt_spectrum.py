...
def compute_cmb_tt_spectrum():
    ...
    The function computes the lensed scalar CMB temperature power spectrum (TT) in units of muK^2
    ...
    # Get lensed scalar CMB power spectra (raw Cl, muK^2)
    powers = results.get_cmb_power_spectra(params=pars, CMB_unit='muK', raw_cl=True, spectra=['lensed_scalar'])
    cl_tt = powers['lensed_scalar'][:, 0]  # TT is first column
    ...
    print("CMB raw temperature power spectrum (TT) computed for l=2 to l=3000.")
